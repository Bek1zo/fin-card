...
def get_person_list(status: bool, page_limit, page_offset) -> dict:
    ...
    with session_scope() as session:
        query = session.query(Person).filter(Person.isActive == status)
        count = query.count()
        ...
        for person in result:
            ...
            if len(person['money_certificate_relation']) > 0:
                cert = person['money_certificate_relation'][-1].money_certificate_relation
            else:
                cert = MoneyCertificateRelation()

            person['money_certificate_id'] = cert.id
            person['status'] = False

            if hasattr(cert, 'report_period_relation') and hasattr(cert.report_period_relation, 'id') and cert.report_period_relation.id != last_period.id:
                person['status'] = False
                person['money_certificate_id'] = None
            else:
                if cert.id:
