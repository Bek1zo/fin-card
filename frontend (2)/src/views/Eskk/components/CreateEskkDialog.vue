...
const moneyCertTransfer = ref(true)

const createEskkDialog = inject('createEskkDialog')
...
const createEskk = () => {
    ...
    if (props.selectedCategory.id === 0) {
        if (props.eskk.value !== null && parseInt(props.eskk.value.split('-')[1]) === (parseInt(props.eskk.value.split('-')[0]) + 1)) {
            regularPost('/eskk/period', {value: props.eskk.value, transfer: moneyCertTransfer.value}).then(async () => {
        ...
        if (props.eskk.name && props.eskk.value) {
            ...
            regularPost('/eskk/' + type, props.eskk).then(async () => {
                emit('update:dialogStatus')
            })
        }}
}
...
<template>
    <Dialog :draggable="false" :pt="{mask: {style: 'backdrop-filter: blur(2px)'}}" v-model:visible="createEskkDialog" :style="{ width: '430px' }" header="Внесение информации в классификатор" :modal="true" class="p-fluid">
        <Fieldset :legend="selectedCategory.name" class="mt-3">
            <div class="formgrid grid">
                ...
                <div class="col-12" v-else>
                    ...
                    <div class="field flex justify-content-center flex-column gap-2">
                        <div class="flex align-items-center gap-2">
                            <InputSwitch id="value" mask="2099-2099" v-model="moneyCertTransfer" :class="{ 'p-invalid': submitted && !eskk.value }"/>
                            <label for="value">Перенос карт из предыдущего периода</label>
                        </div>
                        <small id="value" class="flex justify-content-center">Внимание! Данное действие нельзя отменить!</small>
                    </div>
                </div>
            </div>
        </Fieldset>
        ...
        <template #footer>
            <Button label="Создать" icon="pi pi-check" class="p-button-success" @click="createEskk" />
            <Button label="Закрыть" icon="pi pi-times" class="p-button-danger" @click="hideDialog" />
        </template>
    </Dialog>
</template>
